(function (root, factory) {
  ...
  function readFiniteNumber(value, fallback, name) {
    let number = readNumber(value, fallback);
  ...
  function readPositiveNumber(value, fallback, name) {
    let number = readFiniteNumber(value, fallback, name);
    if (number <= 0) {
  ...
  function readPositiveInteger(value, fallback, name) {
    let number = readInteger(value, fallback, name);
    if (number <= 0) {
      throw new RangeError(name + " must be greater than zero");
    }
  ...
  function readSurfaceMaterial(def) {
    def = def || {};
    let material = def.surfaceMaterial || def.material || {};
    return {
  ...
  function readShapeOptions(def, densityFallback) {
    def = def || {};
    let filter = readFilter(def);
    let material = readSurfaceMaterial(def);
    return {
  ...
  function normalizeVertices(vertices) {
    ...
    if (ArrayBuffer.isView(vertices)) {
      ...
      let typedFlat = Float32Array.from(vertices);
      for (let ti = 0; ti < typedFlat.length; ++ti) {
    ...
    if (typeof vertices[0] === "number") {
      ...
      let arrayFlat = Float32Array.from(vertices);
      for (let ai = 0; ai < arrayFlat.length; ++ai) {
    ...
    let flat = new Float32Array(vertices.length * 2);
  ...
  async function Box2D(options) {
    ...
    let moduleFactory = options.moduleFactory || defaultModuleFactory;
    if (typeof moduleFactory !== "function") {
      throw new Error("Box2D v3 wasm module factory is not available");
    }

    let moduleOptions = Object.assign({}, options.module || {});
    let Module = await moduleFactory(moduleOptions);

    function createWorld(def) {
      def = def || {};
      let gravity = readFiniteVec2(def.gravity, 0, -10, "world.gravity");
      return makeHandle("world", Module._b2js_create_world(gravity.x, gravity.y));
    }

    function createBody(world, def) {
      def = def || {};
      let position = readFiniteVec2(def.position, 0, 0, "body.position");
      let angle = readFiniteNumber(def.angle, 0, "body.angle");
      let type = readInteger(def.type, api.staticBody, "body.type");
    ...
    function createBoxShape(body, def) {
      def = def || {};
      let options = readShapeOptions(def, 1);
      let hx = readPositiveNumber(def.hx == null ? def.halfWidth : def.hx, undefined, "box.hx");
      let hy = readPositiveNumber(def.hy == null ? def.halfHeight : def.hy, undefined, "box.hy");
      return makeHandle(
        "shape",
    ...
    function createCircleShape(body, def) {
      def = def || {};
      let center = readFiniteVec2(def.center, 0, 0, "circle.center");
      let radius = readPositiveNumber(def.radius, undefined, "circle.radius");
      let options = readShapeOptions(def, 1);
      return makeHandle(
        "shape",
    ...
    function createCapsuleShape(body, def) {
      def = def || {};
      let center1 = readFiniteVec2(def.center1 || def.p1, 0, -0.5, "capsule.center1");
      let center2 = readFiniteVec2(def.center2 || def.p2, 0, 0.5, "capsule.center2");
      ensureDistinctPoints(center1, center2, "capsule");
      let radius = readPositiveNumber(def.radius, undefined, "capsule.radius");
      let options = readShapeOptions(def, 1);
      return makeHandle(
        "shape",
    ...
    function createSegmentShape(body, def) {
      def = def || {};
      let p1 = readFiniteVec2(def.p1, 0, 0, "segment.p1");
      let p2 = readFiniteVec2(def.p2, 0, 0, "segment.p2");
      ensureDistinctPoints(p1, p2, "segment");
      let options = readShapeOptions(def, 0);
      return makeHandle(
        "shape",
    ...
    function createPolygonShape(body, def) {
      def = def || {};
      let vertices = normalizeVertices(def.vertices);
      let count = vertices.length / 2;
      validateVertexCount(count, 3, 8, "polygon");
      let ptr = Module._malloc(vertices.byteLength);
      let options = readShapeOptions(def, 1);
    ...
    function createChain(body, def) {
      def = def || {};
      let vertices = normalizeVertices(def.vertices || def.points);
      let count = vertices.length / 2;
    ...
    function readJointAnchors(def) {
      ...
      let anchor = readVec2(def.anchor, 0, 0);
    ...
    function readJointAxis(def, fallbackX, fallbackY) {
      def = def || {};
      if (def.localAxis) {
        let localAxis = readVec2(def.localAxis, fallbackX, fallbackY);
        return { x: localAxis.x, y: localAxis.y, local: true };
      }

      let axis = readVec2(def.axis, fallbackX, fallbackY);
      return { x: axis.x, y: axis.y, local: false };
    }

    function createDistanceJoint(world, bodyA, bodyB, def) {
      def = def || {};
      let anchors = readJointAnchors(def);
      let range = def.lengthRange || {};
      let springForceRange = def.springForceRange || {};
      return makeHandle(
        "joint",
    ...
    function createRevoluteJoint(world, bodyA, bodyB, def) {
      def = def || {};
      let anchors = readJointAnchors(def);
      let referenceAngle = readNumber(def.referenceAngle, 0);
      let localAngleA = readNumber(def.localAngleA, 0);
      let localAngleB = hasOwn(def, "localAngleB") ? readNumber(def.localAngleB, 0) : -referenceAngle;
      return makeHandle(
        "joint",
    ...
    function createPrismaticJoint(world, bodyA, bodyB, def) {
      def = def || {};
      let anchors = readJointAnchors(def);
      let axis = readJointAxis(def, 1, 0);
      return makeHandle(
        "joint",
    ...
    function createWheelJoint(world, bodyA, bodyB, def) {
      def = def || {};
      let anchors = readJointAnchors(def);
      let axis = readJointAxis(def, 0, 1);
      return makeHandle(
        "joint",
    ...
    function createMotorJoint(world, bodyA, bodyB, def) {
      def = def || {};
      let linearVelocity = readVec2(def.linearVelocity, 0, 0);
      return makeHandle(
        "joint",
    ...
    function readFrame(def) {
      def = def || {};
      let position = readVec2(def.position || def.p, 0, 0);
      return {
        x: position.x,
        y: position.y,
    ...
    function getBodyPosition(body) {
      let handle = handleValue(body, "body");
      return {
    ...
    function readBodyTransforms(bodies, out) {
      ...
      let handles = new Int32Array(count);
      for (let i = 0; i < count; ++i) {
        handles[i] = handleValue(bodies[i], "body");
      }

      let output = out || new Float32Array(count * 3);
      ...
      let handlesPtr = Module._malloc(handles.byteLength);
      let outputPtr = Module._malloc(count * 3 * Float32Array.BYTES_PER_ELEMENT);
    ...
    function castRayClosest(world, def) {
      def = def || {};
      let origin = readFiniteVec2(def.origin, 0, 0, "ray.origin");
      let translation = readFiniteVec2(def.translation, 0, 0, "ray.translation");
      let filter = readFilter(def);
      let ptr = Module._malloc(7 * Float32Array.BYTES_PER_ELEMENT);

      try {
        let shapeHandle = Module._b2js_world_cast_ray_closest(
          handleValue(world, "world"),
          origin.x,
          origin.y,
          translation.x,
        ...
        let data = Module.HEAPF32.slice(ptr >> 2, (ptr >> 2) + 7);
    ...
    function overlapAABB(world, def) {
      def = def || {};
      let lower = readFiniteVec2(def.lowerBound || def.lower, 0, 0, "aabb.lowerBound");
      let upper = readFiniteVec2(def.upperBound || def.upper, 0, 0, "aabb.upperBound");
      let filter = readFilter(def);
      let capacity = readPositiveInteger(def.capacity, 64, "overlapAABB.capacity");
      let ptr = Module._malloc(capacity * Int32Array.BYTES_PER_ELEMENT);

      try {
        let count = Module._b2js_world_overlap_aabb(
          handleValue(world, "world"),
          lower.x,
          lower.y,
          upper.x,
        ...
        let handles = Module.HEAP32.subarray(ptr >> 2, (ptr >> 2) + count);
        let results = [];
        for (let i = 0; i < count; ++i) {
    ...
    function readPairEvents(world, countName, fillName, firstName, secondName) {
      let worldHandle = handleValue(world, "world");
      let count = Module[countName](worldHandle);
      if (count <= 0) {
        return [];
      }

      let ptr = Module._malloc(count * 2 * Int32Array.BYTES_PER_ELEMENT);
      try {
        let filled = Module[fillName](worldHandle, ptr, count);
        let handles = Module.HEAP32.subarray(ptr >> 2, (ptr >> 2) + filled * 2);
        let events = [];
    ...
    function getBodyEvents(world) {
      let worldHandle = handleValue(world, "world");
      let count = Module._b2js_world_get_body_event_count(worldHandle);
      if (count <= 0) {
        return [];
      }

      let bodyPtr = Module._malloc(count * Int32Array.BYTES_PER_ELEMENT);
      let transformPtr = Module._malloc(count * 3 * Float32Array.BYTES_PER_ELEMENT);
      let asleepPtr = Module._malloc(count * Int32Array.BYTES_PER_ELEMENT);
      try {
        let filled = Module._b2js_world_get_body_events(worldHandle, bodyPtr, transformPtr, asleepPtr, count);
        let bodies = Module.HEAP32.subarray(bodyPtr >> 2, (bodyPtr >> 2) + filled);
        let transforms = Module.HEAPF32.subarray(transformPtr >> 2, (transformPtr >> 2) + filled * 3);
        let asleep = Module.HEAP32.subarray(asleepPtr >> 2, (asleepPtr >> 2) + filled);
        let events = [];
        for (let i = 0; i < filled; ++i) {
          events.push({
    ...
    function getContactHitEvents(world) {
      let worldHandle = handleValue(world, "world");
      let count = Module._b2js_world_get_contact_hit_count(worldHandle);
      if (count <= 0) {
        return [];
      }

      let shapePtr = Module._malloc(count * 2 * Int32Array.BYTES_PER_ELEMENT);
      let dataPtr = Module._malloc(count * 5 * Float32Array.BYTES_PER_ELEMENT);
      try {
        let filled = Module._b2js_world_get_contact_hit_events(worldHandle, shapePtr, dataPtr, count);
        let shapes = Module.HEAP32.subarray(shapePtr >> 2, (shapePtr >> 2) + filled * 2);
        let data = Module.HEAPF32.subarray(dataPtr >> 2, (dataPtr >> 2) + filled * 5);
        let events = [];
        for (let i = 0; i < filled; ++i) {
          events.push({
    ...
    function getJointEvents(world) {
      let worldHandle = handleValue(world, "world");
      let count = Module._b2js_world_get_joint_event_count(worldHandle);
      if (count <= 0) {
        return [];
      }

      let ptr = Module._malloc(count * Int32Array.BYTES_PER_ELEMENT);
      try {
        let filled = Module._b2js_world_get_joint_events(worldHandle, ptr, count);
        let handles = Module.HEAP32.subarray(ptr >> 2, (ptr >> 2) + filled);
        let events = [];
        for (let i = 0; i < filled; ++i) {
    ...
    function getChainSegments(chain) {
      let handle = handleValue(chain, "chain");
      let count = Module._b2js_chain_get_segment_count(handle);
      if (count <= 0) {
        return [];
      }
      ...
      let ptr = Module._malloc(count * Int32Array.BYTES_PER_ELEMENT);
      try {
        let filled = Module._b2js_chain_get_segments(handle, ptr, count);
        let handles = Module.HEAP32.subarray(ptr >> 2, (ptr >> 2) + filled);
        let segments = [];
        for (let i = 0; i < filled; ++i) {
    ...
    function rayCastShape(shape, def) {
      ...
      let origin = readFiniteVec2(def.origin, 0, 0, "ray.origin");
      let translation = readFiniteVec2(def.translation, 0, 0, "ray.translation");
      let maxFraction = readNonNegativeNumber(def.maxFraction, 1, "ray.maxFraction");
      let ptr = Module._malloc(6 * Float32Array.BYTES_PER_ELEMENT);

      try {
        let hit = Module._b2js_shape_raycast(
        ...
        let data = Module.HEAPF32.subarray(ptr >> 2, (ptr >> 2) + 6);
        return {
          point: { x: data[0], y: data[1] },
          normal: { x: data[2], y: data[3] },
          fraction: data[4],
    ...
    function getShapeAABB(shape) {
      let ptr = Module._malloc(4 * Float32Array.BYTES_PER_ELEMENT);
      try {
        let ok = Module._b2js_shape_get_aabb(handleValue(shape, "shape"), ptr);
        if (!ok) {
          return null;
        }

        let data = Module.HEAPF32.subarray(ptr >> 2, (ptr >> 2) + 4);
        return {
    ...
    function readSurfaceMaterialResult(ok, floatPtr, intPtr) {
      ...
      let floats = Module.HEAPF32.subarray(floatPtr >> 2, (floatPtr >> 2) + 4);
      let ints = Module.HEAP32.subarray(intPtr >> 2, (intPtr >> 2) + 2);
      return {
        friction: floats[0],
    ...
    function getShapeSurfaceMaterial(shape) {
      let floatPtr = Module._malloc(4 * Float32Array.BYTES_PER_ELEMENT);
      let intPtr = Module._malloc(2 * Int32Array.BYTES_PER_ELEMENT);
      try {
    ...
    let api = {
      ...
      setBodyVelocity: function (body, def) {
        def = def || {};
        let velocity = readFiniteVec2(def.linearVelocity || def.velocity, 0, 0, "body.linearVelocity");
      ...
      setShapeFilter: function (shape, filterDef) {
        let filter = readFilter(filterDef);
      ...
      getShapeFilter: function (shape) {
        let handle = handleValue(shape, "shape");
        return {
      ...
      getWorldGravity: function (world) {
        let handle = handleValue(world, "world");
        return {
